...
import { Star, Filter, ChevronDown, Package, Loader2, ShoppingCart, Plus, Check, X, ShieldCheck, Truck, ArrowRight, Minus } from "lucide-react";
...
export default function ProductsPage() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 max-w-7xl mx-auto">
          ...
          {products.map((item) => (
            <div key={item.id} className="group flex flex-col bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl overflow-hidden hover:border-[#D4AF37]/40 transition-all duration-500 shadow-2xl relative">
              <div className="relative h-96 bg-[#050505] overflow-hidden">
                <Image 
                  src={item.metadata?.images?.[0] || item.metadata?.image || "/images/leather_watch.png"} 
                  alt={item.name} 
                  fill 
      ...
      {/* Product Detail Modal (The "Fullness" view) */}
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-black/95 backdrop-blur-xl animate-fade-in">
          <div className="bg-[#0a0a0a] border border-[#222] rounded-[2rem] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-[0_0_100px_rgba(0,0,0,1)] relative">
            ...
            <div className="w-full md:w-1/2 h-[40vh] md:h-auto relative bg-black border-r border-[#222]">
              <Image 
                src={selectedProduct.metadata?.images?.[0] || selectedProduct.metadata?.image || "/images/leather_watch.png"} 
            ...
            <div className="w-full md:w-1/2 p-8 md:p-16 overflow-y-auto custom-scrollbar flex flex-col">
              <div className="mb-10">
                <span className="text-[#D4AF37] uppercase text-[10px] font-black tracking-[0.4em] mb-4 block">Masterpiece Detail</span>
              ...
              <div className="prose prose-invert max-w-none mb-12">
                <p className="text-gray-400 text-lg font-light leading-relaxed">
                  {selectedProduct.description || "Every Wealthyfied timepiece is a statement of power and precision. Meticulously hand-assembled using the world's finest materials, this piece represents the pinnacle of luxury horology."}
                </p>
                ...
                <div className="grid grid-cols-2 gap-8 mt-10">
                   <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h4 className="text-white text-xs font-black uppercase tracking-widest mb-1">Authentic</h4>
                        <p className="text-[10px] text-gray-500">Certified Original</p>
                      </div>
                   </div>
                   <div className="flex items-start gap-4">
                      <Truck className="text-[#D4AF37] shrink-0" size={24} />
                      <div>
                        <h4 className="text-white text-xs font-black uppercase tracking-widest mb-1">Secured</h4>
                        <p className="text-[10px] text-gray-500">Insured Shipping</p>
                      </div>
                   </div>
                </div>
              </div>
              ...
              <div className="mt-auto pt-10 border-t border-[#1a1a1a]">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-8">
                  <div>
                    <span className="text-[10px] text-gray-500 uppercase tracking-widest mb-2 block font-black">Total Acquisition Price</span>
