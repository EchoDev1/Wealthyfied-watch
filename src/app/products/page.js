...
import { Star, Filter, ChevronDown, Package, Loader2 } from "lucide-react";
    ...
    <div className="min-h-screen pt-24 pb-16 px-4 max-w-7xl mx-auto">
      ...
      {isLoading ? (
          ...
          {products.map((item) => (
            <div key={item.id} className="group flex flex-col bg-[#121212] border border-[#222] rounded-xl overflow-hidden hover:border-[#D4AF37]/50 transition-colors">
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-gray-500 mb-3">Ref: {item.sku}</p>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
          ...
          <h2 className="text-xl font-serif text-white">No Watches Available</h2>
          <p className="text-gray-500 max-w-xs">Our vault is currently empty. Our curators are sourcing new masterworks as we speak.</p>
        </div>
      )}
    </div>
  );
